...
public:
     ...
     * 
     * Used in conjunction with constructor that does not set the connection object
     * 
     * @param connection 
     */
    ...
    /*Get Next Output Event
    Gets the next output event from the output event queue. "" if empty*/
...
private:
    ...
    std::vector<uint8_t> calcChecksum(const std::vector<uint8_t>& data, bool sofCheck = true);

    void Graphiti_IO_Write(std::vector<uint8_t> bytes, bool sofCheck = true);
