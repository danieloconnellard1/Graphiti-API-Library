...
/**
 * @brief Uses bit masks to get each digit
 * Then
 * 
 */
...
std::vector<uint8_t> Graphiti_API::calcChecksum(const std::vector<uint8_t>& data, bool sofCheck) {
    int sum = 0;
    for (auto b : data) {
        sum += b;
    }
...
void Graphiti_API::Graphiti_IO_Write(std::vector<uint8_t> bytes, bool sofCheck) {
    auto checksum = calcChecksum(bytes, sofCheck);
...
void Graphiti_API::showMessage(std::string message, uint8_t blinkRate) {
    auto messageBytes = std::vector<uint8_t>(message.begin(), message.end());
    messageBytes.insert(messageBytes.end(), blinkRate);
...
void Graphiti_API::setCursor(uint8_t row, uint8_t column, uint8_t height, uint8_t length, uint8_t blinkRate) {
    Graphiti_IO_Write(startOfFrameCheck({ row, column, height, length, blinkRate }, 0x25));
}
...
void Graphiti_API::getDateAndTime(uint8_t day, uint8_t month, uint16_t year) {
    Graphiti_IO_Write(startOfFrameCheck({ day, month,
...
void Graphiti_API::printVectorHex(const std::vector<uint8_t>& vec) {
    std::cerr << "[ ";
