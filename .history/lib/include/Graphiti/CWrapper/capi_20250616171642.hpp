...
typedef void (*KeyCallback)(const char**, int, void*);
typedef void (*DrawCallback)(void* DrawEvent_C, void*);

bool startUpVCP(GraphitiHandle* handle, std::string port, bool keyEventsBool, bool touchEventsBool);
...
void drawLoop(
    GraphitiHandle* handle,
    std::function<void(const DrawEvent_C&, void*)> draw_func, 
    bool *loop_condition,
    void* parameters
);

int index(GraphitiHandle* handle, int row, int col);
